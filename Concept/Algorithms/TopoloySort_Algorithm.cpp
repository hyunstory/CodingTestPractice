...
이현승
2020년12월05일토요일
이 알고리즘은 위상정렬이라고도 불리며, 한 가지의 답이 안나올 수도 있다. 다만, 우선순위 관계가 있는 노드를 확실히 분리하여
우선순위별로 출력 및 저장 가능한 알고리즘이다. 같은 레벨에 있는 값을 순서 없이 출력하고 싶을 때 사용하면 된다. 
반에서 키가 큰 2명을 여라번 비교한 값으로 대략적인 반의 키 순서를 알 수 있다.
*/
