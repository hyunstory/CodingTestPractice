...
int falldown(int x, int y) {
	...
	for (int i = x + 1; i < r; i++) {
		if (map[i][y] == 'x') {
			if (is_cluster[i][y]) return INF; // 내려갈 칸 개수를 세어야하는데, 자기가 속해있는 클러스트를 선택할 수 있기 때문에, 자기가 속한 클러스트를 만나면 return INF
			else return cnt;
		}
		else if (map[i][y] == '.') cnt++;
	}
...
void reconstruct() {
	...
	for (int i = 0; i < air.size(); i++) {
		...
		else r = min(r, tmp); // 내려갈수 있는 최솟값만. 왜냐하면 다른 미네랄 부분에서 벽에 부딪힐 수 있기 때문에
	}
